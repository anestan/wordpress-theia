...
class WPML_404_Guess extends WPML_Slug_Resolution {
	...
	public function guess_cpt_by_name( $name, $query ) {
		...
		if ( (bool) $types === true ) {
			...
			$post_first = (bool) $query->get( 'name' );

			$cache     = new WPML_WP_Cache( 'WPML_404_Guess' );
			$cache_key = 'guess_cpt' . $name . wp_json_encode( $types ) . $post_first . $date_snippet;
			$found     = false;
			$ret       = $cache->get( $cache_key, $found );

			if ( ! $found ) {
				$ret = $this->find_post_type( $name, $type, $types, $date_snippet, $post_first );
	 ...
	 * @param bool $post_first
	 *
	 * @return array
	 */
	private function find_post_type( $name, $type, $types, $date_snippet, $post_first ) {
		$ret    = array( $name, $type, false );
		...
		$res    = $this->wpdb->get_row(
			"
										 ...
										 WHERE $where
										    ...
										    " . $this->order_by_type_and_language_snippet( (bool) $date_snippet, $post_first ) . '
									     LIMIT 1'
		);
		if ( (bool) $res === true ) {
	 ...
	 * @return string
	 */
	private function order_by_type_and_language_snippet( $has_date, $post_first ) {
		$lang_order   = $this->get_ordered_langs();
		$current_lang = array_shift( $lang_order );
		$best_score   = count( $lang_order ) + 2;
		$order_by     = sprintf( "ORDER BY post_type = 'post' %s", $post_first ? 'DESC' : 'ASC' );
		if ( $best_score > 2 ) {
